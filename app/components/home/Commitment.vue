...
onMounted(async () => {
  ...
  btnMoveX = gsap.quickTo(btnRef.value, "x", { duration: 0.6, ease: "power3.out" });
  btnMoveY = gsap.quickTo(btnRef.value, "y", { duration: 0.6, ease: "power3.out" });
  ...
  tl.from(splitedText1.chars, {
    opacity: 0, y: 100, rotateX: -90, stagger: 0.03, duration: 1, ease: "expo.out",
  })
    .from(charssplit.lines, {
      opacity: 0, y: 30, filter: "blur(10px)", stagger: 0.1, duration: 0.8, ease: "power3.out",
    }, "-=0.6")
    .from(".magnetic-wrap", {
  ...
  tl2.fromTo(".fade", { scale: 1.6 }, { scale: 1, duration: 2.5, ease: "expo.out" })
    .from(spl2.chars, {
      opacity: 0, y: 100, rotateX: -90, stagger: 0.02, duration: 1.2, ease: "expo.out",
    }, "-=2")
    .from(cha2.lines, {
      opacity: 0, y: 40, filter: "blur(10px)", stagger: 0.1, duration: 1, ease: "power3.out",
    }, "-=1.5");
});
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center text-white">
      <div
        ...
        <div class="magnetic-wrap  z-20">
          <div ref="btnRef" class="group p-2 rounded-full border border-white/20 cursor-pointer">
    ...
    <div class="z-0 flex w-full items-center justify-center -mt-43 relative min-h-screen">
      ...
      <div
        ...
        <div
          ...
          Building Fast With <br /> Quality And Speed
        </div>
        <div
          class="cptext2 w-full max-w-[700px] font-[Switzer] text-[clamp(16px,1.8vw,28px)] leading-[1.4] font-normal opacity-90">
          We don't just build; we accelerate. By combining cutting-edge material logistics with a veteran workforce.
        </div>
      </div>
    </div>
  </div>
